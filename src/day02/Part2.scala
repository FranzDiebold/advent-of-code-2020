...
object Part2 {
  ...
  def extractValuesFromLine(line: String): Option[Tuple3[Tuple2[Int, Int], Char, String]] = {
    line match {
        case s"${first}-${second} ${char}: ${password}" => Some(((first.toInt, second.toInt), char(0), password))
        case _ => None
    }
  }
  ...
  def getNumberOfValidPasswords(fileName: String): Int = {
      Source
        ...
        .count({
            case Some(((first: Int, second: Int), char: Char, password: String)) => isPasswordValid(password, char, first, second)
            case None => false
        })
  }
