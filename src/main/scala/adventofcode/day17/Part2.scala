...
object Part2 {
  def readInitialState(fileName: String): Map[Int, Map[Int, Map[Int, Map[Int, Char]]]] = {
    val initialState: Map[Int, Map[Int, Map[Int, Map[Int, Char]]]] =
      Map[Int, Map[Int, Map[Int, Map[Int, Char]]]]()
        .withDefault(_ =>
    ...
    Source
      ...
      .foldLeft[Map[Int, Map[Int, Map[Int, Map[Int, Char]]]]](initialState)({
        case (state: Map[Int, Map[Int, Map[Int, Map[Int, Char]]]], (line: String, y: Int)) =>
          line.zipWithIndex
            .foldLeft[Map[Int, Map[Int, Map[Int, Map[Int, Char]]]]](state)({
              case (
                    currentState: Map[Int, Map[Int, Map[Int, Map[Int, Char]]]],
                    (stateValue: Char, x: Int)
                  ) =>
                setState(currentState, x, y, 0, 0, stateValue)
            })
      })
  }

  def setState(
    state: Map[Int, Map[Int, Map[Int, Map[Int, Char]]]],
    x: Int,
    y: Int,
    z: Int,
    w: Int,
  ...
  def getNumberOfActiveNeighborCubes(
    ...
    (for {
      ...
      .filter({ case (dx: Int, dy: Int, dz: Int, dw: Int) =>
        dx != 0 || dy != 0 || dz != 0 || dw != 0
      })
      .map({ case (dx: Int, dy: Int, dz: Int, dw: Int) =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    val state: Map[Int, Map[Int, Map[Int, Map[Int, Char]]]] =
      performNCycles(initialState, numCycles)
    val numActiveCubes = countActiveCubes(state)
    println(
      s"${numActiveCubes} cubes are left in the active state (in 4 dimensions) after the ${numCycles}th cycle."
    )
  }
}
