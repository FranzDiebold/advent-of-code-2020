...
import day14.Part1.{
  bigIntToFixedLengthBinaryString,
...
object Part2 {
  ...
  def getAllPossibleAddressesFromFloatingBits(addressWithFloatingBits: String): Seq[String] = {
    ...
    (0 to (math.pow(2, numberOfFloatingBits).toInt - 1))
      ...
      .map((floatingBitsValue: String) =>
        floatingBitsValue
          .foldLeft[String](addressWithFloatingBits)((currentAddress: String, bitValue: Char) =>
            currentAddress.replaceFirst("X", bitValue.toString)
          )
      )
  }
        ...
        case (
          ...
          getAllPossibleAddressesFromFloatingBits(memoryAddressWithAppliedBitmask)
            .foldLeft[Map[BigInt, BigInt]](memory) {
              case (currentMemory: Map[BigInt, BigInt], currentMemoryAddress: String) =>
                currentMemory + (binaryStringToBigInt(currentMemoryAddress) -> value)
            }
        }
      }
  }
