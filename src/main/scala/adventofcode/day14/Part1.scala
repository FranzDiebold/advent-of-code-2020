...
object Part1 {
  def readProgram(fileName: String): Iterator[Tuple2[String, Tuple2[BigInt, BigInt]]] = {
    ...
    Source
      ...
      .scanLeft[Tuple2[String, Tuple2[BigInt, BigInt]]](("", (-1, -1)))({
        case ((previousBitmask: String, _), line: String) =>
          line match {
            case bitmaskRegex(bitmask: String) => (bitmask, (-1, -1))
            case writeValueRegex(memoryAddress: String, value: String) =>
              (previousBitmask, (BigInt(memoryAddress), BigInt(value)))
  ...
  def bigIntToFixedLengthBinaryString(number: BigInt, length: Int = 36): String =
    number.toString(2).reverse.padTo(length, '0').reverse
  ...
  def applyBitmask(bitmask: String, value: BigInt): BigInt = {
    val binaryValueString: String = (bitmask zip bigIntToFixedLengthBinaryString(value))
      .map({ case (maskChar: Char, valueChar: Char) =>
        maskChar match {
          case '0' | '1' => maskChar
          case 'X'       => valueChar
        }
  ...
  def runInitializationProgram(
    program: Iterator[Tuple2[String, Tuple2[BigInt, BigInt]]]
  ): Map[BigInt, BigInt] = {
    program
      .foldLeft[Map[BigInt, BigInt]](Map[BigInt, BigInt]()) {
        case (
              memory: Map[BigInt, BigInt],
              (bitmask: String, (memoryAddress: BigInt, value: BigInt))
            ) =>
          memory + (memoryAddress -> applyBitmask(bitmask, value))
      }
  }
