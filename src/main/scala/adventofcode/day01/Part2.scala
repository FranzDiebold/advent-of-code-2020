...
object Part2 {
  def getSpecialNumberTriple(
    fileName: String,
    targetValue: Int
  ): Option[Option[Tuple3[Int, Int, Int]]] = {
    Source
        ...
        case ((Some(previousValue), allPreviousValues, allPreviousPairSumsMap), currentValue) => {
          (
            ...
            allPreviousValues.foldLeft(allPreviousPairSumsMap) {
              (previousMap: Map[Int, Int], currentPrevValue: Int) =>
                previousMap + ((previousValue + currentPrevValue) -> previousValue)
            }
          )
        }
      ...
      .find({
        case (Some(currentValue), _, allPreviousPairSumsMap) =>
          allPreviousPairSumsMap contains (targetValue - currentValue)
        case (None, _, _) => false
      })
      .map({
        case (Some(currentValue), _, allPreviousPairSumsMap) =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    val triple: Option[Option[Tuple3[Int, Int, Int]]] =
      getSpecialNumberTriple(fileName, targetValue)

    triple match {
      case Some(Some(tripleValue)) =>
        println(
          s"The three entries that sum to $targetValue are $tripleValue. Their product is ${tripleValue._1 * tripleValue._2 * tripleValue._3}."
        )
