...
object Part2 {
  ...
  def findOutputOfTerminatingProgram(program: IndexedSeq[Option[Tuple2[String, Int]]], instructionIdxToChange: Int = 0): Option[Int] = {
    ...
    program(instructionIdxToChange) match {
      case Some((operation: String, argument: Int)) => operationChangeMap.keySet.contains(operation) match {
        case true => {
          val updatedProgram = program.updated(instructionIdxToChange, Some(operationChangeMap(operation), argument))
  ...
  def main(args: Array[String]): Unit = {
    ...
    findOutputOfTerminatingProgram(program) match {
      case Some(programResult: Int) => println(s"The value of the accumulator after the program terminates is ${programResult}.")
